...
public class AddAddressInfo extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AddAddressInfo frame = new AddAddressInfo();
	...
	public AddAddressInfo() {
		...
		JLabel label_5 = new JLabel("添加地址");
		...
		label_4.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(textArea.getText().length()!=0||textField.getText().length()!=0){
					...
					if(UserService.addAddressService(userID, new Address(sex,address,phone))){
						JOptionPane.showMessageDialog(AddAddressInfo.this, "添加成功!","提示",JOptionPane.INFORMATION_MESSAGE);
						AddAddressInfo.this.dispose();
						new MainPage().setVisible(true);
					}else{
						JOptionPane.showMessageDialog(AddAddressInfo.this, "添加失败!","提示",JOptionPane.INFORMATION_MESSAGE);
					}
				}else{
					JOptionPane.showMessageDialog(AddAddressInfo.this, "不能为空!","提示",JOptionPane.INFORMATION_MESSAGE);
				}
			}
		});
		...
		contentPane.addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseDragged(MouseEvent e){
				Point p = AddAddressInfo.this.getLocation();
				AddAddressInfo.this.setLocation(p.x+e.getX()-origin.x, p.y+e.getY()-origin.y);
			}
